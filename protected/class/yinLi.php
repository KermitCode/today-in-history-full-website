...
public function __construct($today=''){
	...
	if(!$today) $this->today=time();
	
	else $this->today=strtotime($today);
	...
	#获得当日日期 
	$today=getdate($this->today); 
	
	if($today["year"]<1901 || $today["year"]>2020) die("年份范围出错！"); 
	...
	$this->yl_year=$mten[$everymonth[$j][14]].$mtwelve[$everymonth[$j][15]];
	...
	$this->week_char=$weekday[$cur_wday];
