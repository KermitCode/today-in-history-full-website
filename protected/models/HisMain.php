...
class HisMain extends CActiveRecord
{
	...
	public function rules()
	{
		...
		return array(
			array('ls_id, ls_title, ls_year, ls_day, ls_cont', 'required'),
			array('ls_id, ls_class, ls_nongnummonth, ls_nongnumday, publiced', 'numerical', 'integerOnly'=>true),
			array('ls_title', 'length', 'max'=>100),
			array('ls_englishtit', 'length', 'max'=>255),
			array('ls_year, ls_nongyear, ls_nongmonth, ls_nongday, ls_time', 'length', 'max'=>10),
			array('ls_month, ls_day', 'length', 'max'=>2),
			// The following rule is used by search().
			// Please remove those attributes that should not be searched.
			array('ls_id, ls_title, ls_englishtit, ls_class, ls_year, ls_month, ls_day, ls_nongyear, ls_nongmonth, ls_nongnummonth, ls_nongday, ls_nongnumday, ls_cont, publiced, ls_time', 'safe', 'on'=>'search'),
		);
	}
