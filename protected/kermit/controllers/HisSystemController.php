...
class HisSystemController extends Controller{
	...
	public function actionmodify($type=''){
		...
		switch($type){
				...
				case 'super':
					...
					$data=Yii::app()->db->createCommand("select * from his_system")->query()->read();
					...
					if(md5($super1)!=$data['superadmin_pass']) $this->kermitBase->msg_show('原密码错误！');
						...
						Yii::app()->db->createCommand("update his_system set superadmin_pass='{$pass}'")->query();
				...
				case 'normal':
					...
					Yii::app()->db->createCommand("update his_system set adminer_pass='{$password}'")->query();
				...
				case 'test':
					...
					Yii::app()->db->createCommand("update his_system set adtest_pass='{$password}'")->query();
...
//kermit:for web all set
	...
	public function actionWebset(){
			...
			if(isset($_GET['modify']) && $_GET['modify']=='modify'){//修改网站设置
					...
					$sql="update his_system set ".implode(',',$sql_array);
